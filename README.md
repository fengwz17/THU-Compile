...
最后一步，添加对数组的支持和指针算术，包括数组的声明、数组和指针的下标，指针加减整数、整数加指针、指针减指针。

这一步语法的扩展比较容易，但是由于引入数组，增加一个数组类型进行类型检查，需要考虑各种情况，总体来说比较繁琐，并且这一步的样例测试中发现在前面许多步中为了通过样例测试而写的检查函数会不再适用，需要进行修改。

这一步仍然参考了助教示例代码中的符号表等结构和整体函数框架，并且从中学习了C++类和指针的一些用法，维护一个全局的符号表，进行类型检查。

## 错误检查
主要依赖全局符号表中维护的变量类型等信息，由于需要检查的情况较多，主要通过对无法正确报错的错误样例，增加对该错误情况的考虑。
